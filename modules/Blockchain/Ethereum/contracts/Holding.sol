...
import {DictionaryStorage} from './DictionaryStorage.sol';
...
contract Holding is Ownable {
    ...
    uint256 holdingTimeInMinutes, uint256 tokenAmountPerHolder, uint256 dataSetSizeInBytes, uint256 litigationIntervalInMinutes) public {
        ...
        if(DictionaryStorage(hub.getContractAddress("DictionaryStorage")).fingerprint(bytes32(dataSetId)) == bytes32(0)){
            DictionaryStorage(hub.getContractAddress("DictionaryStorage")).setFingerprint(bytes32(dataSetId), bytes32(dataRootHash));
        } else {
            require(bytes32(dataRootHash) == DictionaryStorage(hub.getContractAddress("DictionaryStorage")).fingerprint(bytes32(dataSetId)),
        ...
        uint256 difficulty;
        if(DictionaryStorage(hub.getContractAddress("DictionaryStorage")).getDifficultyOverride() != 0) {
            difficulty = DictionaryStorage(hub.getContractAddress("DictionaryStorage")).getDifficultyOverride();
        }
        else {
    ...
    internal {
        DictionaryStorage dictionaryStorage = DictionaryStorage(hub.getContractAddress("DictionaryStorage"));

        dictionaryStorage.setOfferParameter(offerId, "Creator", bytes32(creator));
        dictionaryStorage.setOfferParameter(offerId, "DataSetId", bytes32(dataSetId));
        dictionaryStorage.setOfferParameter(offerId, "HoldingTimeInMinutes", bytes32(holdingTimeInMinutes));
        dictionaryStorage.setOfferParameter(offerId, "TokenAmountPerHolder", bytes32(tokenAmountPerHolder));
        dictionaryStorage.setOfferParameter(offerId, "LitigationIntervalInMinutes", bytes32(litigationIntervalInMinutes));
        dictionaryStorage.setOfferParameter(offerId, "Task", bytes32(task));
        dictionaryStorage.setOfferParameter(offerId, "Difficulty", bytes32(difficulty));
    }
    ...
    internal {
        ...
        dictionaryStorage.setOfferParameter(offerId, "RedLitigationHash", redLitigationHash);
        dictionaryStorage.setOfferParameter(offerId, "GreenLitigationHash", greenLitigationHash);
        dictionaryStorage.setOfferParameter(offerId, "BlueLitigationHash", blueLitigationHash);
    }
    ...
    public {
        ...
        require(bytes32(identity) == dictionaryStorage.getOfferParameter(bytes32(offerId), "Creator"), "Offer can only be finalized by its creator!");
        ...
        hashes[0] = keccak256(abi.encodePacked(holderIdentity[0], dictionaryStorage.getOfferParameter(bytes32(offerId), "Task")));
        hashes[1] = keccak256(abi.encodePacked(holderIdentity[1], dictionaryStorage.getOfferParameter(bytes32(offerId), "Task")));
        hashes[2] = keccak256(abi.encodePacked(holderIdentity[2], dictionaryStorage.getOfferParameter(bytes32(offerId), "Task")));

        require(uint256(hashes[0]) < uint256(hashes[1]) && uint256(hashes[1]) < uint256(hashes[2]), "Solution hashes are not sorted!");

        // Verify task answer
        require(((keccak256(abi.encodePacked(hashes[0], hashes[1], hashes[2])) >> (shift * 4)) & bytes32((2 ** (4 * uint256(dictionaryStorage.getOfferParameter(bytes32(offerId), "Difficulty")))) - 1))
            == dictionaryStorage.getOfferParameter(bytes32(offerId), "Task"), "Submitted identities do not answer the task correctly!");

        // Write data into storage
        setHolders(bytes32(offerId), holderIdentity, encryptionType);
    ...
    internal {
        ...
        uint256 amount = uint256(DictionaryStorage(hub.getContractAddress("DictionaryStorage")).getOfferParameter(bytes32(offerId), "TokenAmountPerHolder"));
    ...
    function setHolders(bytes32 offerId, address[] identities, uint8[] litigationEncryptionTypes)
    internal {
        DictionaryStorage dictionaryStorage = DictionaryStorage(hub.getContractAddress("DictionaryStorage"));
        
        bytes32 stakedAmount = dictionaryStorage.getOfferParameter(offerId, "TokenAmountPerHolder");
        
        dictionaryStorage.setHolderParameter(offerId, identities[0], "StakedAmount", stakedAmount);
        dictionaryStorage.setHolderParameter(offerId, identities[0], "PaymentTimestamp", bytes32(block.timestamp));
        dictionaryStorage.setHolderParameter(offerId, identities[0], "LitigationEncryptionType", bytes32(litigationEncryptionTypes[0]));
        
        dictionaryStorage.setHolderParameter(offerId, identities[1], "StakedAmount", stakedAmount);
        dictionaryStorage.setHolderParameter(offerId, identities[1], "PaymentTimestamp", bytes32(block.timestamp));
        dictionaryStorage.setHolderParameter(offerId, identities[1], "LitigationEncryptionType", bytes32(litigationEncryptionTypes[1]));

        dictionaryStorage.setHolderParameter(offerId, identities[2], "StakedAmount", stakedAmount);
        dictionaryStorage.setHolderParameter(offerId, identities[2], "PaymentTimestamp", bytes32(block.timestamp));
        dictionaryStorage.setHolderParameter(offerId, identities[2], "LitigationEncryptionType", bytes32(litigationEncryptionTypes[2]));
    }
